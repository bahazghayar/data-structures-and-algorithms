...
const validateWord = (word) => {
  // Solution code here...
  let reg3 =  /^[a-zA-Z]{5,10}$/  ; 

  return reg3.test(word) ;
};
...
const hasNumber = (string) => {
  // Solution code here...
  let reg4 =  /^([A-Za-z]+(\d)+)|[A-Za-z]+(\d)+[\w]$/   ;
 
  return reg4.test(string); 
};
...
const validateEmail = (email) => {
  // Solution code here...
  let reg5 = /^([\w]+|[\w]+\.[\w]+)@([\w]+\.)+((com)|(net)|(org))$/ ; 
   return reg5.test(email) ;
};
...
const validatePhoneNumber = (phoneNumber) => {
  // Solution code here...
  let reg6 = /^((\(\d{3}\))|\d{3})(\s|\-)?\d{3}(\s|\-)?\d{4}$/;
  return reg6.test(phoneNumber);  
};
