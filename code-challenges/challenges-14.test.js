...
const screenForNames = (arr) => {
  ...
  let result = arr.filter(str => regex.test(str));
...
const toTitleCase = (arr) => {
  // Solution code here...
  return arr.map(str => str.charAt(0).toUpperCase() + str.substring(1, 8));
};
...
let biggerThanLuke = (arr) => {
  // Solution code here...
  let biggerChar = arr.filter(char => char.mass > 77).map(char => char.name).join(' - ');
  return biggerChar;
};
